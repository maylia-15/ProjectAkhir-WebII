...
<body class="bg-[#F8FAFC] text-slate-800 font-sans antialiased h-screen w-screen overflow-hidden">
    <div class="flex h-full w-full overflow-hidden relative">
        ...
        <aside id="sidebar" class="hidden md:flex flex-col justify-between w-64 bg-white border-r border-slate-200 h-full shrink-0 z-30 shadow-sm ml-0 relative">
            <div>
                <div class="px-7 py-8 flex items-center border-b border-slate-100/50">
            ...
            <div class="fixed inset-y-0 left-0 w-64 bg-white flex flex-col justify-between h-full shadow-2xl">
                <div>
                    <div class="px-6 py-6 flex items-center justify-between border-b border-slate-100">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 bg-[#1a8e5f] rounded-lg flex items-center justify-center text-white font-bold">B</div>
                            <span class="font-bold text-slate-900 text-xl">BISA</span>
                        </div>
                        <button onclick="closeMobileDrawer()" class="text-slate-400 p-1.5 cursor-pointer"><i class="fa-solid fa-xmark text-xl"></i></button>
                    </div>
                ...
                <div class="p-4 border-t border-slate-100 flex flex-col gap-2">
                    <a href="{{ route('profile.show') }}" class="w-full flex items-center justify-center gap-2 py-3 bg-slate-50 text-slate-700 font-bold rounded-xl text-sm hover:bg-slate-100"><i class="fa-solid fa-circle-user"></i> Profil Saya</a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="w-full flex items-center justify-center gap-2 py-3.5 bg-rose-50 text-rose-600 font-bold rounded-xl text-sm hover:bg-rose-100 cursor-pointer"><i class="fa-solid fa-door-open"></i> Keluar</button>
                    </form>
                </div>
            </div>
        </div>
